...
function [x,f] = optSolver_Team_Beekman(problem,method,options)
...
while k < options.max_iterations && norm_g >= options.term_tol*max(1,norm_g_0)
    ...
    init_f = init_f + func_evals;
    init_g = init_g + grad_evals;
...
disp("Num_iterations: " + k)
disp("Num_func_evals: " + init_f)
disp("Num_grad_eval: " + init_g)

figure(1);
plot(1:k, fplot(1:k), 'LineWidth',2);
%semilogy(1:k, fplot(1:k) - problem.opt_val, 'LineWidth', 2); 
%semilogy(1:k, (fplot(1:k) - problem.opt_val), 'LineWidth', 2);
%semilogy(1:k, fplot(1:k), 'LineWidth', 2);
...
Plot different step sizes based on the chosen method

if contains(method.name, 'TR')
    % Plot with a dashed line if method contains "TR"
    plot_style = '--';
elseif strcmp(method.options.step_type, 'Backtracking')
    % Plot with a solid line if using backtracking
    plot_style = '-';
else
    % Plot with a dotted line if using Wolfe
    plot_style = ':';
end

% figure(1);
% semilogy(1:k, fplot(1:k) - problem.opt_val, plot_style, 'LineWidth',2);
% xlabel('Iterations $k$', 'Interpreter', 'latex');
% ylabel('$f(x_k) - f^*$', 'Interpreter', 'latex');
% grid on;
% hold on;
...
%% PLotting different stepsize

% figure(2);
% plot(1:actual_iterations, a_bar_values_all, 'LineWidth', 2);
% xlabel('Iteration');
% ylabel('alpha');
% title('Change of stepsize over iterations');
% grid on;
% hold on;
% set(gca, 'FontName', 'Times New Roman', 'FontSize', 16);
